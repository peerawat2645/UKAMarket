  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: List.generate(4, (j) {
        return FutureBuilder(
          future:
              BaseClient().getStoreDescription('/reserv/choice/row/$j/col/17'),
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.done) {
              Map<String, dynamic> jsonMap = json.decode(snapshot.data);
              if (jsonMap['body'] == "success") {
                              ...
                              ElevatedButton.icon(
                                onPressed: () {
                                  ...
                                  Navigator.push(
                                    ...
                                    MaterialPageRoute(
                                        builder: (context) =>
                                            ReservationInfo(row:j,column:17,userId: userId, storeId: storeId, type: type, startDate: startDate, storetype: storetype, endDate: endDate, month: month, price: price)),
                                  ); // Close the dialog
                                },
                                icon: const Icon(
                                  Icons
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: List.generate(4, (j) {
        return FutureBuilder(
          future: BaseClient().getStoreDescription(
              '/reserv/choice/month/$month/row/$j/col/17'),
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.done) {
              Map<String, dynamic> jsonMap = json.decode(snapshot.data);
              if (jsonMap['body'] == "fail") {
